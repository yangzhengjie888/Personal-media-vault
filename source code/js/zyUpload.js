		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input name="upfile" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
					html += '				<span id="fileDragArea" class="upload_drag_area">or drag files to this box</span>';
					html += '			</div>';
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">selected 0 files，in total 0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<div class="webuploader_pick">Continue</div>';
		            html += '					<div class="upload_btn">Start</div>';
		            html += '				</div>';
		            html += '			</div>';
					html += '			<div id="preview" class="upload_preview"></div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">continue and upload</button>';
					html += '		</div>';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">selected 0 file(s)，in total 0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<input name="upfile" type="file" size="30" name="fileselect[]" '+multiple+'>';
				    ...
				    html += '					<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" title="Add files" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				$("#status_info").html("selected "+num+" file(s), in total "+size+".");
			};
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 51200000) {
						alert('The "'+ file.name +'" is so large, Please try a small one');	
					} else {
						// 在这里需要判断当前所有文件中
						arrFiles.push(file);	
					}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var delHtml = "";
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del" data-index="'+file.index+'" title="delect"></span>';
				}
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">Fail! Please try again...</p>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onDelete: function(file, files) {
						...
						console.info("The rest of files");
						console.info(files);
					},
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>Successful, address is: " + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
					...
					onFailure: function(file) {
						$("#uploadProgress_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						$("#uploadInf").append("<p>File: " + file.name + " upload error!</p>");	
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						alert("Please select the file before upload it");
					}
	            });
